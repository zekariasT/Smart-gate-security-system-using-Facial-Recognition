...
def train_faces():
    ...
    for users in os.listdir("static\TrainingData\\"):
        names.append(users)

    # Get the path to all the images
    for name in names:
        for image in os.listdir("static\TrainingData\{}".format(name)):
            path_string = os.path.join("static\TrainingData\{}".format(name), image)
